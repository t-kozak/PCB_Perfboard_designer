...
export interface IProjectSave {
   ...
   * solder side stateless: no `lines`/`nets` are written, colour/width live on
   * each connection, and `routingMode` is persisted (§11).
   */
  version?: number;
  ...
  ICs: Ic[];
  placedIcs?: any[];
  /** Legacy: persisted net layer (v3 and earlier). Read only for colour migration. */
  nets?: INet[];
