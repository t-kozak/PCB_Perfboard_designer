...
pads, add notes, and export a board as a PNG or KiCad netlist, or the whole multi-board
project as one file.
...
| `src/features/description.ts` | Add/remove a text note on a component (`prompt()`-based) — the Add Note / Remove Note button (no shortcut). |
| `src/features/duplicate.ts` | `d` — duplicate the selected component(s) plus every connection *between* them. Copies keep rotation/`config`/notes, get fresh ids + next free designators, and follow the cursor via `startIcDragAt` (listed in `State.placingDuplicates`) until a board click drops them as one undo entry. Escape / Delete / any click off the canvas cancels (copies removed, nothing recorded); autosave skips while placing. |
...
| `src/kicad/` | **Pure** project-file format — a KiCad S-expression netlist. `sexpr.ts` (reader/writer), `project-file.ts` (the multi-board `.perfboard` wrapper — `writeProjectFile()` / `parseProjectFile()` / `isProjectFile()`), `netlist.ts` (`parseNetlist()` / `writeNetlist()`: `comp`s with `Perfboard:<id>` footprints + `property`s, `nets`, and a `(perfboard …)` block for board size/labels/routing/pad colours/custom parts; per-component `perfboard:at` / `perfboard:note` / `perfboard:pins` properties), `export.ts` (`snapshotToNetlist()` — board snapshot → netlist; unique refs, bridges `BRn`), `project.ts` (`planProject()` — netlist → board plan: part resolution incl. parametric `dip-N` / `sip-N` / `pin-header-RxC` and KiCad symbol/footprint/value guessing, pin mapping by `pinfunction` then number, value/properties → `config`, auto-placement of unpositioned parts via `placement.ts`, and **one MST of connections per net**), `placement.ts` (`placeComponents()` — connectivity-aware auto-placement: groups from `perfboard:group` / KiCad `sheetpath` (else grown round every 6+-pin part), greedy wire-length placement + re-insertion inside each group, then groups as blocks by `(perfboard (floorplan …))` rows or least added wire; power rails down-weighted; `perfboard:edge` hints). |
| `src/features/project/` | Boards, export/import, persistence. `boards.ts` — the **multi-board project**: a list of board slots, exactly one open, and the open one *is* `State` (so no other feature knows boards exist). Switching away parks the board's live `State` slice (dots, components, connections, nets, grid modes, routing, undo stack, board size) in its slot plus its netlist text, so switching back is lossless and keeps undo; a board not yet opened this session loads from its text. `switchBoard` / `addBoard` / `renameBoard` / `removeBoard` (never the last; removing the open board opens its neighbour) / `newProject` / `importText` (a `.perfboard` project file replaces the project; a single `.net` or legacy `.json` is added as a new board, named from the netlist's design `source` or the file name — or fills the lone empty board of a fresh project) / `serializeProject()`; fires `boards-changed`. `boards-ui.ts` — the Boards sidebar section (below Project): one row per board — click opens it, double-click renames it inline, trash icon deletes it (confirmed), the title's ＋ adds one and opens its name for editing. `project-menu.ts` — the Project sidebar section: the Export drop-down (board → PNG, board → KiCad `.net`, project → `.perfboard`), Import (button or drop a file anywhere on the page). `save-project.ts` (`getSaveNetlist(date?, title?)` — snapshot of the open board → netlist text, `title` = board name as the design `source`; `downloadText()`), `load-project.ts` (`loadBoardText()` — netlist via `planProject()` + `applyPlan()`, or a **legacy JSON** save via `loadProject()`: a v1/v2 file migrates each hand-drawn wire to one `IConnection` carrying that wire's `width`, a bridge per bare-hole endpoint; `finishLoad()` — selection reset + derived-state rebuild after `State` is swapped), `autosave.ts` (debounced localStorage `project` write of the whole project file from `redrawCanvas()`), `load-from-local-storage.ts` (auto-restore on `DOMContentLoaded` — the `project` key, else a pre-multi-board `save` key (netlist or legacy JSON) as the only board — then `armAutosave()`), `parts-library.ts` (Parts Library download), `reset-project.ts` (New Project), `resize-grid.ts`, `undo-redo.ts` (batch history over **connections + components** only — `IChange`, `recordChange()`, matched by `id`; wires are not tracked; per board), `save-image.ts` (`downloadAsImage()`, `p` shortcut). |
...
- Placed-component ids are `crypto.randomUUID()` strings (secure-context only —
  ...
  save file: each board is saved as a **KiCad netlist** (`src/kicad/`), keyed by
  reference designator, and a project (`.perfboard`, `src/kicad/project-file.ts`)
  is one `(perfboard_project (active …) (board (name …) (export …)) …)`
  S-expression wrapping every board's netlist verbatim. It stores nets, not connections, so a load rebuilds each
